        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="font-display flex items-center gap-2">
              ...
              {detailModal?.type === 'marketCap' && 'Market Cap Details'}
              {detailModal?.type === 'fdv' && 'Fully Diluted Valuation'}
              {detailModal?.type === 'volume' && '24h Volume Analysis'}
              {detailModal?.type === 'liquidity' && 'Liquidity Details'}
              {detailModal?.type === 'holders' && 'Holder Statistics'}
              {detailModal?.type === 'transactions' && 'Transaction Analysis'}
              {detailModal?.type === 'pair' && 'Trading Pair Details'}
              {detailModal?.type === 'pool' && 'Liquidity Pool Details'}
              {detailModal?.type === 'holder' && 'Holder Information'}
              {detailModal?.type === 'trend' && 'AI Trend Analysis'}
              {detailModal?.type === 'risk' && 'Risk Assessment'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {detailModal?.type === 'marketCap' && (
              <>
                <p className="text-sm text-muted-foreground">Market capitalization is calculated by multiplying the current price by the circulating supply.</p>
                <div className="p-4 rounded-lg bg-muted/30">
                  <div className="text-2xl font-bold">{formatNumber(detailModal.data.marketCap)}</div>
                  <div className="text-sm text-muted-foreground mt-2">
                    Price: {formatPrice(token.price)} × Circulating Supply
                  </div>
                </div>
                <Button variant="outline" className="w-full gap-2" onClick={() => window.open(`https://www.coingecko.com/en/coins/${token.symbol.toLowerCase()}`, '_blank')}>
                  <ExternalLink className="w-4 h-4" /> View on CoinGecko
                </Button>
              </>
            )}
            {detailModal?.type === 'volume' && (
              <>
                <p className="text-sm text-muted-foreground">Total trading volume across all exchanges in the last 24 hours.</p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-muted/30">
                    <div className="text-xs text-muted-foreground">24h Volume</div>
                    <div className="font-bold">{formatNumber(detailModal.data.volume24h)}</div>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/30">
                    <div className="text-xs text-muted-foreground">Volume Change</div>
                    <div className={cn("font-bold", detailModal.data.volumeChange >= 0 ? "text-success" : "text-danger")}>
                      {detailModal.data.volumeChange >= 0 ? "+" : ""}{detailModal.data.volumeChange.toFixed(2)}%
                    </div>
                  </div>
                </div>
                <Button variant="outline" className="w-full gap-2" onClick={() => window.open(`https://dexscreener.com/${chain.id}/${token.contractAddress}`, '_blank')}>
                  <ExternalLink className="w-4 h-4" /> View on DexScreener
                </Button>
              </>
            )}
            {detailModal?.type === 'pair' && (
              <>
                <div className="p-4 rounded-lg bg-muted/30">
                  <div className="font-bold text-lg">{detailModal.data.pair}</div>
                  <div className="text-sm text-muted-foreground">{detailModal.data.dex}</div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-muted/30">
                    <div className="text-xs text-muted-foreground">24h Volume</div>
                    <div className="font-bold">{formatNumber(detailModal.data.volume)}</div>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/30">
                    <div className="text-xs text-muted-foreground">Liquidity</div>
                    <div className="font-bold">{formatNumber(detailModal.data.liquidity)}</div>
                  </div>
                </div>
                <Button variant="outline" className="w-full gap-2" onClick={() => window.open(`https://dexscreener.com/${chain.id}/${token.contractAddress}`, '_blank')}>
                  <ExternalLink className="w-4 h-4" /> Trade on DEX
                </Button>
              </>
            )}
            {detailModal?.type === 'holder' && (
              <>
                <div className="p-4 rounded-lg bg-muted/30">
                  <div className="text-xs text-muted-foreground mb-1">Wallet Address</div>
                  <code className="text-sm font-mono text-primary break-all">{detailModal.data.address}</code>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-muted/30">
                    <div className="text-xs text-muted-foreground">Balance</div>
                    <div className="font-bold">{detailModal.data.balance}</div>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/30">
                    <div className="text-xs text-muted-foreground">% of Supply</div>
                    <div className="font-bold">{detailModal.data.pct}%</div>
                  </div>
                </div>
                <Button variant="outline" className="w-full gap-2" onClick={() => window.open(`${chain.explorer}/address/${detailModal.data.address}`, '_blank')}>
                  <ExternalLink className="w-4 h-4" /> View on Explorer
                </Button>
              </>
            )}
            {!['marketCap', 'volume', 'pair', 'holder'].includes(detailModal?.type || '') && (
              <p className="text-sm text-muted-foreground">Detailed information for this metric. Click external links for more data.</p>
            )}
          </div>
        </DialogContent>
      </Dialog>
