    ...
    <>
      ...
      <Dialog open={riskModalOpen} onOpenChange={setRiskModalOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          {riskModalData?.type === 'methodology' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/10 border border-border/30">
                ...
                <p className="text-sm text-muted-foreground">
                  Our AI analyzes multiple factors to calculate a comprehensive risk score from 0-100.
                </p>
              </div>
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  ...
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Volatility Analysis</h5>
                    <p className="text-xs text-muted-foreground">Measures price swings and stability over time</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <BarChart3 className="h-5 w-5 text-secondary mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Liquidity Score</h5>
                    <p className="text-xs text-muted-foreground">Evaluates trading depth and exit capability</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <Zap className="h-5 w-5 text-warning mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Volume Analysis</h5>
                    <p className="text-xs text-muted-foreground">Detects unusual spikes and manipulation patterns</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <Lock className="h-5 w-5 text-success mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Contract Security</h5>
                    <p className="text-xs text-muted-foreground">Checks for honeypots, ownership, and vulnerabilities</p>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-4 gap-2">
                <div className="p-2 rounded-lg bg-success/10 text-center">
                  ...
                  <p className="text-sm font-medium text-success">0-24</p>
                </div>
                <div className="p-2 rounded-lg bg-warning/10 text-center">
                  <p className="text-xs text-muted-foreground">Medium</p>
                  <p className="text-sm font-medium text-warning">25-49</p>
                </div>
                <div className="p-2 rounded-lg bg-danger/10 text-center">
                  <p className="text-xs text-muted-foreground">High</p>
                  <p className="text-sm font-medium text-danger">50-74</p>
                </div>
                <div className="p-2 rounded-lg bg-destructive/10 text-center">
                  <p className="text-xs text-muted-foreground">Extreme</p>
                  <p className="text-sm font-medium text-destructive">75-100</p>
                </div>
              </div>
            </div>
          )}

          {riskModalData?.type === 'overview' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-xl bg-muted/10 text-center">
                  <p className="text-xs text-muted-foreground">Total Tokens</p>
                  <p className="text-2xl font-display text-foreground">{riskModalData.data.totalTokens}</p>
                </div>
                <div className="p-4 rounded-xl bg-muted/10 text-center">
                  <p className="text-xs text-muted-foreground">Avg Risk</p>
                  <p className="text-2xl font-display text-warning">{riskModalData.data.avgRiskScore.toFixed(0)}</p>
                </div>
              </div>
              <div className="p-4 rounded-xl bg-primary/10 border border-primary/30">
                <p className="text-sm text-foreground">
                  Based on our analysis, the {chain.name} ecosystem shows a 
                  {riskModalData.data.avgRiskScore < 30 ? " healthy" : riskModalData.data.avgRiskScore < 60 ? " moderate" : " elevated"} 
                  risk profile with {riskModalData.data.lowRiskCount} safe tokens and {riskModalData.data.highRiskCount} requiring caution.
                </p>
              </div>
            </div>
          )}
