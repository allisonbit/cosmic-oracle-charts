    ...
    <>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          {modalData?.type === 'inflow' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-success/10 border border-success/30">
                <p className="text-2xl font-display text-success">{formatValue(modalData.data.value)}</p>
                <p className="text-sm text-muted-foreground">{modalData.data.percent.toFixed(1)}% of total capital flow</p>
              </div>
              <div className="space-y-3">
                <h4 className="text-sm font-medium text-foreground">Inflow Sources</h4>
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-muted/10">
                    <p className="text-xs text-muted-foreground">From CEXs</p>
                    <p className="text-lg font-display text-foreground">{formatValue(modalData.data.value * 0.45)}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/10">
                    <p className="text-xs text-muted-foreground">From DeFi</p>
                    <p className="text-lg font-display text-foreground">{formatValue(modalData.data.value * 0.35)}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/10">
                    <p className="text-xs text-muted-foreground">From Bridges</p>
                    <p className="text-lg font-display text-foreground">{formatValue(modalData.data.value * 0.15)}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/10">
                    <p className="text-xs text-muted-foreground">Other</p>
                    <p className="text-lg font-display text-foreground">{formatValue(modalData.data.value * 0.05)}</p>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground">
                  <Info className="h-3 w-3 inline mr-1" />
                  Inflow indicates capital entering the {chain.name} ecosystem, typically bullish
                </p>
              </div>
            </div>
          )}

          {modalData?.type === 'outflow' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-danger/10 border border-danger/30">
                <p className="text-2xl font-display text-danger">{formatValue(modalData.data.value)}</p>
                <p className="text-sm text-muted-foreground">{modalData.data.percent.toFixed(1)}% of total capital flow</p>
              </div>
              <div className="space-y-3">
                <h4 className="text-sm font-medium text-foreground">Outflow Destinations</h4>
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-muted/10">
                    <p className="text-xs text-muted-foreground">To CEXs</p>
                    <p className="text-lg font-display text-foreground">{formatValue(modalData.data.value * 0.5)}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/10">
                    <p className="text-xs text-muted-foreground">To Other Chains</p>
                    <p className="text-lg font-display text-foreground">{formatValue(modalData.data.value * 0.3)}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/10">
                    <p className="text-xs text-muted-foreground">To Stablecoins</p>
                    <p className="text-lg font-display text-foreground">{formatValue(modalData.data.value * 0.15)}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/10">
                    <p className="text-xs text-muted-foreground">Other</p>
          ...
          {modalData?.type === 'netflow' && (
            <div className="space-y-4">
              <div className={cn("p-4 rounded-xl border", modalData.data.value >= 0 ? "bg-success/10 border-success/30" : "bg-danger/10 border-danger/30")}>
                <p className={cn("text-2xl font-display", modalData.data.value >= 0 ? "text-success" : "text-danger")}>
                  {modalData.data.value >= 0 ? "+" : ""}{formatValue(modalData.data.value)}
                </p>
                <p className="text-sm text-muted-foreground">{modalData.data.value >= 0 ? "Net Bullish" : "Net Bearish"} Signal</p>
              </div>
              <div className="space-y-3">
                <div className="flex justify-between p-3 rounded-lg bg-muted/10">
                  <span className="text-sm text-muted-foreground">Total Inflow</span>
                  <span className="text-sm text-success font-medium">{formatValue(modalData.data.inflow)}</span>
                </div>
                <div className="flex justify-between p-3 rounded-lg bg-muted/10">
                  <span className="text-sm text-muted-foreground">Total Outflow</span>
                  <span className="text-sm text-danger font-medium">{formatValue(modalData.data.outflow)}</span>
                </div>
                <div className="p-3 rounded-lg bg-primary/10 border border-primary/30">
                  <p className="text-xs text-muted-foreground mb-1">Market Interpretation</p>
                  <p className="text-sm text-foreground">
                    {modalData.data.value >= 0 
                      ? "Positive net flow indicates accumulation. Smart money is moving capital into the ecosystem, suggesting bullish sentiment."
                      : "Negative net flow indicates distribution. Capital is leaving the ecosystem, suggesting profit-taking or risk-off sentiment."
                    }
                  </p>
                </div>
              </div>
          ...
          {modalData?.type === 'wallet' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/10 border border-border/30">
                <p className="text-sm text-muted-foreground mb-1">Wallet Address</p>
                <div className="flex items-center gap-2">
                  <p className="font-mono text-foreground">{modalData.data.address}</p>
                  <button onClick={() => copyToClipboard(modalData.data.address)} className="text-muted-foreground hover:text-foreground">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 rounded-lg bg-muted/10 text-center">
                  ...
                  <p className="text-lg font-display text-foreground">{formatValue(modalData.data.balance)}</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/10 text-center">
                  <p className="text-xs text-muted-foreground">PnL</p>
                  <p className={cn("text-lg font-display", modalData.data.pnl >= 0 ? "text-success" : "text-danger")}>
                    {modalData.data.pnl >= 0 ? "+" : ""}{formatValue(modalData.data.pnl)}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-muted/10 text-center">
                  <p className="text-xs text-muted-foreground">Trades</p>
                  <p className="text-lg font-display text-foreground">{modalData.data.trades}</p>
                </div>
              </div>
              <div className="flex gap-2">
                <a
                  href={`${chain.explorerUrl}/address/${modalData.data.address}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-primary/20 text-primary hover:bg-primary/30 transition-colors text-sm"
                >
                ...
                <a
                  href={`https://debank.com/profile/${modalData.data.address}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-muted/20 text-muted-foreground hover:text-foreground transition-colors text-sm"
                >
                  <ExternalLink className="h-4 w-4" /> DeBank
                </a>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/10 border border-border/30">
                <div className="flex items-center justify-center gap-4 mb-3">
                  <span className="text-xl font-display text-foreground">{modalData.data.from}</span>
                  <ArrowRightLeft className="h-5 w-5 text-primary" />
                  <span className="text-xl font-display text-foreground">{modalData.data.to}</span>
                </div>
                <p className="text-2xl font-display text-center text-primary">{formatValue(modalData.data.amount)}</p>
              </div>
              <div className="flex gap-2">
                <a
                  href={getDexScreenerLink(modalData.data.to)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-primary/20 text-primary hover:bg-primary/30 transition-colors text-sm"
                >
                  <BarChart3 className="h-4 w-4" /> View Chart
                </a>
                <a
                  href={getUniswapLink()}
                  target="_blank"
