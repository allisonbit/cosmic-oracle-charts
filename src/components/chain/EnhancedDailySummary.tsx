    ...
    <>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          {modalData?.type === 'signals' && modalData.data.signal && (
            <div className="space-y-4">
              <div className={cn("p-4 rounded-xl border", modalData.data.signal.bgColor, `border-${modalData.data.signal.color.replace('text-', '')}/30`)}>
                <div className="flex items-center gap-3 mb-3">
                  {(() => {
                    const Icon = modalData.data.signal.icon;
                    return <Icon className={cn("h-8 w-8", modalData.data.signal.color)} />;
                  })()}
                  <div>
                    <h4 className="text-xl font-display text-foreground">{modalData.data.signal.signal}</h4>
                    <p className={cn("text-sm", modalData.data.signal.color)}>
                      {modalData.data.signal.score.toFixed(0)}% confidence
                    </p>
                  </div>
                </div>
                <Progress value={modalData.data.signal.score} className="h-2" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-muted/10">
                  <p className="text-xs text-muted-foreground">Confidence</p>
                  <p className="text-lg font-display text-foreground">{modalData.data.metrics.confidence}%</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/10">
                  <p className="text-xs text-muted-foreground">Data Points</p>
                  <p className="text-lg font-display text-foreground">{modalData.data.metrics.dataPoints}</p>
                </div>
              </div>
              <div className="p-3 rounded-lg bg-primary/10 border border-primary/30">
                <p className="text-sm text-foreground">
                  Based on {modalData.data.metrics.dataPoints} data points, our AI model has determined a {modalData.data.signal.signal.toLowerCase()} market outlook with {modalData.data.metrics.confidence}% confidence.
                </p>
              </div>
            </div>
          )}
          ...
          {modalData?.type === 'methodology' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/10 border border-border/30">
                <h4 className="font-medium text-foreground mb-2">AI Analysis Process</h4>
                <p className="text-sm text-muted-foreground">
                  Our AI analyzes multiple data sources to generate comprehensive market summaries.
                </p>
              </div>
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  ...
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Price & Volume Analysis</h5>
                    <p className="text-xs text-muted-foreground">Real-time price movements and trading volume patterns</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <MessageCircle className="h-5 w-5 text-secondary mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Social Sentiment</h5>
                    <p className="text-xs text-muted-foreground">Twitter, Reddit, and community discussions</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Crypto news and mainstream media coverage</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <Activity className="h-5 w-5 text-success mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">On-Chain Metrics</h5>
                    <p className="text-xs text-muted-foreground">Whale movements, exchange flows, active addresses</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {modalData?.type === 'insights' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/10 border border-border/30">
                <p className="text-lg font-display text-foreground">{modalData.title}</p>
              </div>
              {modalData.data.time && (
              ...
              {modalData.data.sentiment && (
                <div className={cn(
                  "p-3 rounded-lg",
                  modalData.data.sentiment === 'positive' ? "bg-success/10" :
                  modalData.data.sentiment === 'negative' ? "bg-danger/10" : "bg-warning/10"
                )}>
                  <p className={cn(
                    "text-sm font-medium",
                    modalData.data.sentiment === 'positive' ? "text-success" :
                    modalData.data.sentiment === 'negative' ? "text-danger" : "text-warning"
                  )}>
                    {modalData.data.sentiment.charAt(0).toUpperCase() + modalData.data.sentiment.slice(1)} Sentiment
                  </p>
                </div>
              )}
            </div>
