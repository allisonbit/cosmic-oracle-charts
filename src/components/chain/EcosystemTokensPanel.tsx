            ...
            <div className="space-y-5 py-4">
              ...
              <div className="flex items-end justify-between p-4 rounded-xl bg-muted/10 border border-border/30">
                <div>
                  ...
                  <p className="text-3xl font-display text-foreground">
                    ${selectedToken.price.toFixed(selectedToken.price > 10 ? 2 : 6)}
                  </p>
                </div>
                <div className={cn(
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="p-3 rounded-lg bg-muted/10 border border-border/30">
                  ...
                  <p className="text-lg font-semibold text-foreground">{formatSimple(selectedToken.holders)}</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/10 border border-border/30">
                  <div className="flex items-center gap-2 text-muted-foreground mb-1">
              ...
              {selectedToken.address !== "Native" && (
                <div className="p-3 rounded-lg bg-muted/10 border border-border/30">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="text-xs text-foreground bg-muted/30 px-2 py-1 rounded flex-1 truncate">
                      {selectedToken.address}
                    </code>
                    <button
                      onClick={(e) => copyAddress(selectedToken.address, e)}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <a
                  href={getDexScreenerUrl(selectedToken.address)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 p-3 rounded-lg bg-primary/20 text-primary hover:bg-primary/30 transition-all text-sm"
                >
                  <BarChart3 className="h-4 w-4" />
                  View Chart
                </a>
              ...
              <div className="flex flex-wrap gap-2">
                <a
                  href={`https://www.coingecko.com/en/coins/${selectedToken.symbol.toLowerCase()}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                <a
                  href={`https://dextools.io/app/en/${chain.dexScreenerId || chain.id}/pair-explorer`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted/20 text-muted-foreground hover:text-foreground transition-all text-xs"
                >
                  DexTools <ExternalLink className="h-3 w-3" />
                </a>
                <a
                  href={`https://tokensniffer.com/token/${chain.id}/${selectedToken.address}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted/20 text-muted-foreground hover:text-foreground transition-all text-xs"
                >
                  <Shield className="h-3 w-3" /> Security Check <ExternalLink className="h-3 w-3" />
                </a>
              </div>

              {/* Disclaimer */}
              <p className="text-[10px] text-muted-foreground/60 text-center pt-2 border-t border-border/20">
                Data is for informational purposes only. Always DYOR before trading.
              </p>
            </div>
          )}
        </DialogContent>
