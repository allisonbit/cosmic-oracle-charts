  ...
  return (
    <div className="holo-card p-4 sm:p-6">
      ...
      <Dialog open={!!selectedDetail} onOpenChange={() => setSelectedDetail(null)}>
        <DialogContent className="max-w-lg border-border/50 bg-card/95 backdrop-blur-xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 font-display">
              {selectedDetail?.type === "timeframe" && <Clock className="h-5 w-5 text-primary" />}
              {selectedDetail?.type === "trigger" && <AlertTriangle className="h-5 w-5 text-warning" />}
              {selectedDetail?.type === "risk" && <Shield className="h-5 w-5 text-primary" />}
              {selectedDetail?.type === "confidence" && <Brain className="h-5 w-5 text-primary" />}
          ...
          <div className="space-y-4">
            {/* Timeframe Detail */}
            {selectedDetail?.type === "timeframe" && (
              <>
                <div className="flex items-center justify-between p-4 rounded-xl bg-muted/20">
                  <div>
                    <p className="text-sm text-muted-foreground">{selectedDetail.data.timeframe}</p>
                    <p className={cn(
                      "text-2xl font-display capitalize",
                      selectedDetail.data.prediction === "bullish" && "text-success",
                      selectedDetail.data.prediction === "bearish" && "text-danger",
                      selectedDetail.data.prediction === "neutral" && "text-warning"
                    )}>
                      {selectedDetail.data.prediction}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-muted-foreground">Confidence</p>
                    <p className="text-2xl font-display text-primary">{selectedDetail.data.confidence}%</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{selectedDetail.data.description}</p>
                </div>
                ...
                <div>
                  ...
                  <ul className="space-y-1.5">
                    {selectedDetail.data.methodology.map((method: string, i: number) => (
                ...
                <div>
                  <h4 className="text-sm font-medium text-foreground mb-2">Reasoning</h4>
                  <p className="text-sm text-muted-foreground">{selectedDetail.data.reasoning}</p>
                </div>

                {selectedDetail.data.priceTarget > 0 && (
                  <div className="p-3 rounded-xl bg-primary/10 border border-primary/20">
                    <p className="text-xs text-muted-foreground">Price Target</p>
                    <p className="text-xl font-display text-primary glow-text">
                      ${selectedDetail.data.priceTarget.toLocaleString()}
                    </p>
                  </div>
                )}
              </>
            ...
            {/* Trigger Detail */}
            {selectedDetail?.type === "trigger" && (
              <>
                <div className={cn(
                  "p-4 rounded-xl",
                  selectedDetail.data.severity === "high" && "bg-danger/10 border border-danger/20",
                  selectedDetail.data.severity === "medium" && "bg-warning/10 border border-warning/20",
                  selectedDetail.data.severity === "low" && "bg-success/10 border border-success/20"
                )}>
                  <div className="flex items-center gap-2 mb-2">
                    <selectedDetail.data.icon className={cn(
                      "h-5 w-5",
                      selectedDetail.data.severity === "high" && "text-danger",
                      selectedDetail.data.severity === "medium" && "text-warning",
                      selectedDetail.data.severity === "low" && "text-success"
                    )} />
                    <span className="font-medium text-foreground">{selectedDetail.data.trigger}</span>
                  </div>
                  <span className={cn(
                    "text-xs px-2 py-0.5 rounded-full capitalize",
                    selectedDetail.data.severity === "high" && "bg-danger/20 text-danger",
                    selectedDetail.data.severity === "medium" && "bg-warning/20 text-warning",
                    selectedDetail.data.severity === "low" && "bg-success/20 text-success"
                  )}>
                    {selectedDetail.data.severity} impact
                  </span>
                </div>
                ...
                <div>
                  <h4 className="text-sm font-medium text-foreground mb-2">Historical Impact</h4>
                  <p className="text-sm text-muted-foreground">{selectedDetail.data.impact}</p>
                </div>
                ...
                <div className="p-3 rounded-xl bg-primary/10 border border-primary/20">
                  ...
                  <p className="text-sm text-muted-foreground">{selectedDetail.data.actionable}</p>
                </div>
              </>
            )}

            {/* Risk Detail */}
            {selectedDetail?.type === "risk" && (
              <>
                <div className="flex items-center justify-between p-4 rounded-xl bg-muted/20">
                  <div>
                    ...
                    <p className={cn(
                      ...
                      selectedDetail.data.riskLevel < 30 && "text-success",
                      selectedDetail.data.riskLevel >= 30 && selectedDetail.data.riskLevel < 60 && "text-warning",
                      selectedDetail.data.riskLevel >= 60 && "text-danger"
                    )}>
                      {selectedDetail.data.riskLevel}/100
                    </p>
                  </div>
                  <div className={cn(
                    "px-3 py-1.5 rounded-full text-sm font-medium",
                    selectedDetail.data.riskLevel < 30 && "bg-success/20 text-success",
                    selectedDetail.data.riskLevel >= 30 && selectedDetail.data.riskLevel < 60 && "bg-warning/20 text-warning",
                    selectedDetail.data.riskLevel >= 60 && "bg-danger/20 text-danger"
                  )}>
                    {selectedDetail.data.riskLevel < 30 ? "Low Risk" : selectedDetail.data.riskLevel < 60 ? "Medium Risk" : "High Risk"}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {selectedDetail.data.factors.map((factor: any, i: number) => (
                      <div key={i}>
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="text-foreground">{factor.name}</span>
                          <span className={cn(
                            factor.score < 30 && "text-success",
                            factor.score >= 30 && factor.score < 60 && "text-warning",
                            factor.score >= 60 && "text-danger"
                        ...
                        <div className="h-1.5 bg-muted/50 rounded-full overflow-hidden">
                          <div 
                            className={cn(
                              "h-full rounded-full",
                              factor.score < 30 && "bg-success",
                              factor.score >= 30 && factor.score < 60 && "bg-warning",
                              factor.score >= 60 && "bg-danger"
                            )}
                ...
                <div>
                  ...
                  <ul className="space-y-1.5">
                    {selectedDetail.data.recommendations.map((rec: string, i: number) => (
                      <li key={i} className="flex items-start gap-2 text-xs text-muted-foreground">
            ...
            {/* Confidence Detail */}
            {selectedDetail?.type === "confidence" && (
              <>
                  ...
                  <div className="relative">
                    <svg className="w-32 h-32 transform -rotate-90">
                      ...
                      <circle
                        ...
                        strokeDasharray={`${(selectedDetail.data.overall / 100) * 351.8} 351.8`}
                        className="text-primary"
                        style={{ filter: "drop-shadow(0 0 10px hsl(var(--primary) / 0.6))" }}
                      />
                    </svg>
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-3xl font-display text-primary">{selectedDetail.data.overall}%</span>
                ...
                <div>
                  <h4 className="text-sm font-medium text-foreground mb-3">Confidence Breakdown</h4>
                  <div className="space-y-3">
                    {selectedDetail.data.breakdown.map((item: any, i: number) => (
                      <div key={i}>
                        <div className="flex items-center justify-between text-xs mb-1">
                ...
                <div className="p-3 rounded-xl bg-muted/20">
                  ...
                  <div className="space-y-1.5 text-xs text-muted-foreground">
                    <p><span className="text-foreground">Models Used:</span> {selectedDetail.data.modelInfo.models.join(", ")}</p>
                    <p><span className="text-foreground">Data Points:</span> {selectedDetail.data.modelInfo.dataPoints}</p>
                    <p><span className="text-foreground">Update Frequency:</span> {selectedDetail.data.modelInfo.lastUpdated}</p>
                  </div>
                </div>
              </>
            )}
