    ...
    <>
            ...
            <TabsContent value="galaxy">
              ...
              <div className="relative h-64 rounded-xl overflow-hidden bg-background/50 border border-border/30">
                <svg viewBox="0 0 400 200" className="w-full h-full">
                  ...
                  {/* Labels */}
                  <text x="100" y="170" textAnchor="middle" fill="hsl(160 84% 39%)" fontSize="14" fontFamily="Orbitron">
                    Positive
                  </text>
                  <text x="200" y="170" textAnchor="middle" fill="hsl(38 92% 50%)" fontSize="14" fontFamily="Orbitron">
                    Neutral
                  </text>
                  <text x="300" y="170" textAnchor="middle" fill="hsl(0 84% 60%)" fontSize="14" fontFamily="Orbitron">
                    Negative
                  </text>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          {modalData?.type === 'platform' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/10 border border-border/30">
                <div className="flex items-center gap-3 mb-3">
                  {platforms.find(p => p.key === modalData.data.platform)?.icon && (
                    <div className={cn("p-2 rounded-lg", platforms.find(p => p.key === modalData.data.platform)?.bgColor)}>
                      {(() => {
                        const Icon = platforms.find(p => p.key === modalData.data.platform)?.icon;
                        return Icon ? <Icon className={cn("h-5 w-5", platforms.find(p => p.key === modalData.data.platform)?.color)} /> : null;
                      })()}
                    </div>
                  )}
                  <h4 className="text-lg font-display text-foreground">
                    {platforms.find(p => p.key === modalData.data.platform)?.label}
                  </h4>
                </div>
                <p className="text-sm text-muted-foreground">
                  {"volume" in modalData.data ? `${modalData.data.volume.toLocaleString()} total mentions` : `${modalData.data.count} articles`}
                </p>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 rounded-lg bg-success/10 text-center">
                  <p className="text-xs text-muted-foreground">Positive</p>
                  <p className="text-xl font-display text-success">{modalData.data.positive.toLocaleString()}</p>
                  <p className="text-xs text-success/70">
                    {((modalData.data.positive / (modalData.data.positive + modalData.data.neutral + modalData.data.negative)) * 100).toFixed(1)}%
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-warning/10 text-center">
                  <p className="text-xs text-muted-foreground">Neutral</p>
                  <p className="text-xl font-display text-warning">{modalData.data.neutral.toLocaleString()}</p>
                  <p className="text-xs text-warning/70">
                    {((modalData.data.neutral / (modalData.data.positive + modalData.data.neutral + modalData.data.negative)) * 100).toFixed(1)}%
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-danger/10 text-center">
                  <p className="text-xs text-muted-foreground">Negative</p>
                  <p className="text-xl font-display text-danger">{modalData.data.negative.toLocaleString()}</p>
                  <p className="text-xs text-danger/70">
                    {((modalData.data.negative / (modalData.data.positive + modalData.data.neutral + modalData.data.negative)) * 100).toFixed(1)}%
                  </p>
                </div>
              </div>
              <div className="p-3 rounded-lg bg-primary/10 border border-primary/30">
                <p className="text-sm text-foreground">
                  {modalData.data.positive > modalData.data.negative 
                    ? "Overall sentiment is positive. The community is showing bullish signals on this platform."
                    : "Overall sentiment is negative. Exercise caution as community sentiment appears bearish."
                  }
                </p>
              </div>
            </div>
          ...
          {modalData?.type === 'influencer' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/10 border border-border/30 flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-primary/20 flex items-center justify-center">
                  <Users className="h-8 w-8 text-primary" />
                </div>
                <div>
                  <h4 className="text-lg font-display text-foreground">{modalData.data.name}</h4>
                  <p className="text-sm text-muted-foreground">{modalData.data.followers} followers</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-muted/10">
                  ...
                  <p className={cn(
                    ...
                    modalData.data.sentiment === "Bullish" ? "text-success" :
                    modalData.data.sentiment === "Neutral" ? "text-warning" : "text-danger"
                  )}>
                    {modalData.data.sentiment}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-muted/10">
                  <p className="text-xs text-muted-foreground">Engagement</p>
                  <p className="text-lg font-display text-foreground">{modalData.data.engagement}%</p>
                </div>
              </div>
              <a
                href={`https://twitter.com/${modalData.data.name.replace('@', '')}`}
                target="_blank"
                rel="noopener noreferrer"
              ...
              >
                <Twitter className="h-4 w-4" /> View Profile
              </a>
            </div>
          )}

          {modalData?.type === 'trending' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-primary/10 border border-primary/30">
                <div className="flex items-center gap-2 mb-2">
                  <Hash className="h-5 w-5 text-primary" />
                  <h4 className="text-lg font-display text-foreground">{modalData.data.topic}</h4>
                </div>
                <p className="text-sm text-muted-foreground">{modalData.data.mentions.toLocaleString()} mentions</p>
              </div>
              <div className="p-3 rounded-lg bg-muted/10">
                <p className="text-xs text-muted-foreground mb-2">Sentiment Score</p>
                <div className="flex items-center gap-3">
                  <Progress value={modalData.data.sentiment} className="flex-1 h-3" />
                  <span className={cn(
                    "font-medium",
                    modalData.data.sentiment >= 55 ? "text-success" : modalData.data.sentiment >= 45 ? "text-warning" : "text-danger"
                  )}>
                    {modalData.data.sentiment.toFixed(0)}%
                  </span>
                </div>
              </div>
              <div className="flex gap-2">
                <a
                  href={`https://twitter.com/search?q=${encodeURIComponent(modalData.data.topic)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                >
                  <Twitter className="h-4 w-4" /> Search Twitter
                </a>
              </div>
            </div>
          )}
          ...
          {modalData?.type === 'methodology' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/10 border border-border/30">
                <h4 className="font-medium text-foreground mb-2">Sentiment Analysis</h4>
                <p className="text-sm text-muted-foreground">
                  We use AI-powered natural language processing to analyze social media posts, news articles, and community discussions.
                </p>
              </div>
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <Twitter className="h-5 w-5 text-[#1DA1F2] mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Twitter/X Analysis</h5>
                    <p className="text-xs text-muted-foreground">Real-time tweets, engagement metrics, influencer tracking</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <MessageCircle className="h-5 w-5 text-[#FF4500] mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Reddit Analysis</h5>
                    <p className="text-xs text-muted-foreground">Subreddit posts, comments, upvote ratios</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <Newspaper className="h-5 w-5 text-warning mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">News Analysis</h5>
                    <p className="text-xs text-muted-foreground">Crypto news sites, mainstream media coverage</p>
                  </div>
                </div>
              </div>
            </div>
