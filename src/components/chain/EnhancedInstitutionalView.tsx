    ...
    <>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          {modalData?.type === 'etf' && !Array.isArray(modalData.data) && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-primary/10 border border-primary/30">
                <h4 className="text-lg font-display text-foreground">{modalData.data.etf}</h4>
                <p className="text-2xl font-display text-primary mt-2">{formatNumber(modalData.data.totalAum)}</p>
                <p className="text-sm text-muted-foreground">Assets Under Management</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-muted/10">
                  <p className="text-xs text-muted-foreground">24h Flow</p>
                  <p className={cn("text-lg font-display", modalData.data.flow24h >= 0 ? "text-success" : "text-danger")}>
                    {modalData.data.flow24h >= 0 ? "+" : ""}{formatNumber(modalData.data.flow24h)}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-muted/10">
                  <p className="text-xs text-muted-foreground">7d Flow</p>
                  <p className={cn("text-lg font-display", modalData.data.flow7d >= 0 ? "text-success" : "text-danger")}>
                    {modalData.data.flow7d >= 0 ? "+" : ""}{formatNumber(modalData.data.flow7d)}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-muted/10 col-span-2">
                  <p className="text-xs text-muted-foreground">Price Correlation</p>
                  <p className="text-lg font-display text-foreground">{(modalData.data.correlation * 100).toFixed(1)}%</p>
                  <Progress value={modalData.data.correlation * 100} className="h-2 mt-2" />
                </div>
              </div>
              <div className="p-3 rounded-lg bg-muted/10">
                <p className="text-sm text-foreground">
                  {modalData.data.flow24h > 0 
                    ? "Positive inflows indicate institutional buying pressure and growing demand."
                    : "Negative outflows suggest institutional profit-taking or rotation to other assets."
                  }
                </p>
              </div>
            </div>
          ...
          {modalData?.type === 'validator' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-xl bg-muted/10">
                  <p className="text-xs text-muted-foreground">Total Validators</p>
                  <p className="text-2xl font-display text-foreground">{modalData.data.totalValidators.toLocaleString()}</p>
                </div>
                <div className="p-4 rounded-xl bg-muted/10">
                  <p className="text-xs text-muted-foreground">Staking Yield</p>
                  <p className="text-2xl font-display text-success">{modalData.data.stakingYield.toFixed(2)}%</p>
                </div>
                <div className="p-4 rounded-xl bg-muted/10">
                  <p className="text-xs text-muted-foreground">MEV Revenue</p>
                  <p className="text-2xl font-display text-foreground">{formatNumber(modalData.data.mevRevenue)}</p>
                </div>
                <div className="p-4 rounded-xl bg-muted/10">
                  <p className="text-xs text-muted-foreground">Avg Balance</p>
                  <p className="text-2xl font-display text-foreground">{modalData.data.avgValidatorBalance.toFixed(2)} ETH</p>
                </div>
              </div>
              <div className="p-4 rounded-xl bg-warning/10 border border-warning/30">
                <p className="text-xs text-muted-foreground mb-1">Break-Even Price</p>
                <p className="text-xl font-display text-warning">${modalData.data.breakEvenPrice.toLocaleString()}</p>
                <p className="text-sm text-muted-foreground mt-2">
                  Validators remain profitable as long as {chain.symbol} stays above this price.
                </p>
              </div>
            </div>
          )}

          {modalData?.type === 'holdings' && !Array.isArray(modalData.data) && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/30">
                <h4 className="text-lg font-display text-foreground">{modalData.data.holder}</h4>
                <p className="text-sm text-muted-foreground">Institutional Holder</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-muted/10">
                  <p className="text-xs text-muted-foreground">Holdings</p>
                  <p className="text-lg font-display text-foreground">{modalData.data.holdings.toLocaleString()} {chain.symbol}</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/10">
                  <p className="text-xs text-muted-foreground">% of Supply</p>
                  <p className="text-lg font-display text-foreground">{modalData.data.percentSupply.toFixed(2)}%</p>
                </div>
              </div>
              <div className={cn(
                "p-3 rounded-lg",
                modalData.data.change30d >= 0 ? "bg-success/10" : "bg-danger/10"
              )}>
                <p className="text-xs text-muted-foreground">30d Change</p>
                <p className={cn("text-lg font-display", modalData.data.change30d >= 0 ? "text-success" : "text-danger")}>
                  {modalData.data.change30d >= 0 ? "+" : ""}{modalData.data.change30d.toFixed(2)}%
                </p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/10 border border-border/30">
                ...
                <p className="text-sm text-muted-foreground">
                  Institutional data is aggregated from SEC filings, ETF providers, and on-chain analytics.
                </p>
              </div>
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  ...
                  <div>
                    <h5 className="text-sm font-medium text-foreground">ETF Flow Data</h5>
                    <p className="text-xs text-muted-foreground">Daily inflow/outflow from major ETF providers</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <Building2 className="h-5 w-5 text-amber-400 mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Institutional Holdings</h5>
                    <p className="text-xs text-muted-foreground">13F filings and public announcements</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/10">
                  <BarChart3 className="h-5 w-5 text-orange-400 mt-0.5" />
                  <div>
                    <h5 className="text-sm font-medium text-foreground">Futures Data</h5>
                    <p className="text-xs text-muted-foreground">CME Group and derivatives exchanges</p>
                  </div>
                </div>
              </div>
            </div>
