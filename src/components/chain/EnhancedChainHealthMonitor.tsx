            ...
            <>
              ...
              <div className="space-y-6">
                ...
                <div className="p-4 rounded-xl bg-primary/10 border border-primary/20 text-center">
                  <p className="text-3xl font-bold text-foreground mb-2">{selectedMetric.value}</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {selectedMetric.details.map((detail, index) => (
                    ...
                    >
                      ...
                      {detail.info && (
                        <p className="text-[10px] text-muted-foreground mt-1">{detail.info}</p>
                      )}
                    </div>
                  ))}
                </div>
                ...
                {/* External Links */}
                {selectedMetric.links && selectedMetric.links.length > 0 && (
                  <div className="flex flex-wrap gap-2 pt-4 border-t border-border/30">
                    {selectedMetric.links.map((link, index) => (
                      <a
                        key={index}
                        href={link.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-4 py-2 rounded-lg bg-primary/20 text-primary text-sm hover:bg-primary/30 transition-colors flex items-center gap-2"
                      >
