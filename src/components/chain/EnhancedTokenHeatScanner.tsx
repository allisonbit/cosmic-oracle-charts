        ...
        <DialogContent className="sm:max-w-lg bg-background/95 backdrop-blur-xl border-border/50 max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              <div className={cn(
                "p-2 rounded-lg",
                selectedToken?.heatLevel === "hot" && "bg-danger/20",
                selectedToken?.heatLevel === "warm" && "bg-warning/20",
                selectedToken?.heatLevel === "cool" && "bg-primary/20"
              )}>
                {selectedToken?.heatLevel === "hot" ? <Flame className="h-5 w-5 text-danger" /> : <Activity className="h-5 w-5 text-primary" />}
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xl font-display">{selectedToken?.symbol}</span>
                  <Badge className={cn(
                    selectedToken?.heatLevel === "hot" && "bg-danger/20 text-danger",
                    selectedToken?.heatLevel === "warm" && "bg-warning/20 text-warning",
                    selectedToken?.heatLevel === "cool" && "bg-primary/20 text-primary"
                  )}>
                    {selectedToken?.heatLevel?.toUpperCase()}
                  </Badge>
                </div>
          ...
          {selectedToken && (
            <div className="space-y-4 py-4">
              {/* Price & Change */}
              <div className="flex items-end justify-between p-4 rounded-lg bg-muted/10 border border-border/30">
                <div>
                  <p className="text-xs text-muted-foreground">Current Price</p>
                  <p className="text-3xl font-bold text-foreground">{formatPrice(selectedToken.price)}</p>
                </div>
                <div className="text-right">
                  <div className={cn(
                    ...
                    {selectedToken.change24h >= 0 ? <TrendingUp className="h-5 w-5" /> : <TrendingDown className="h-5 w-5" />}
              ...
              {/* Heat Score */}
              <div className="p-4 rounded-lg bg-gradient-to-r from-danger/10 via-warning/10 to-primary/10 border border-border/30">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm font-medium text-foreground">Heat Score</span>
                  <span className={cn(
                    "text-2xl font-bold",
                    selectedToken.heatLevel === "hot" && "text-danger",
                    selectedToken.heatLevel === "warm" && "text-warning",
                    selectedToken.heatLevel === "cool" && "text-primary"
                  )}>
                    {selectedToken.heatScore.toFixed(0)}/100
                  </span>
                </div>
                <Progress value={selectedToken.heatScore} className={cn(
                  "h-3",
                  selectedToken.heatLevel === "hot" && "[&>div]:bg-danger",
                  selectedToken.heatLevel === "warm" && "[&>div]:bg-warning",
                  selectedToken.heatLevel === "cool" && "[&>div]:bg-primary"
                )} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-muted/10 border border-border/30">
                  ...
                  <p className="text-xl font-bold text-foreground">{selectedToken.momentum.toFixed(0)}%</p>
                  <Progress value={selectedToken.momentum} className="h-1.5 mt-2" />
                </div>
                <div className="p-3 rounded-lg bg-muted/10 border border-border/30">
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xl font-bold text-foreground">{selectedToken.volumeSpike.toFixed(0)}%</p>
                  <Progress value={selectedToken.volumeSpike} className="h-1.5 mt-2 [&>div]:bg-secondary" />
                </div>
                <div className="p-3 rounded-lg bg-muted/10 border border-border/30">
                  <div className="flex items-center gap-2 mb-2">
                    <Users className="h-4 w-4 text-success" />
                    <span className="text-xs text-muted-foreground">Social Score</span>
                  </div>
                  <p className="text-xl font-bold text-foreground">{selectedToken.socialScore.toFixed(0)}%</p>
                  <Progress value={selectedToken.socialScore} className="h-1.5 mt-2 [&>div]:bg-success" />
                </div>
                <div className="p-3 rounded-lg bg-muted/10 border border-border/30">
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xl font-bold text-foreground">{selectedToken.volatility.toFixed(0)}%</p>
                  <Progress value={selectedToken.volatility} className="h-1.5 mt-2 [&>div]:bg-warning" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-lg bg-success/5 border border-success/30">
                  ...
                  <p className={cn("text-lg font-bold", getSignalStrength(selectedToken).color)}>
                    {getSignalStrength(selectedToken).strength}
                  </p>
                </div>
                <div className={cn(
                  "p-4 rounded-lg border",
                  getRiskLevel(selectedToken).color === "text-danger" && "bg-danger/5 border-danger/30",
                  getRiskLevel(selectedToken).color === "text-warning" && "bg-warning/5 border-warning/30",
                  getRiskLevel(selectedToken).color === "text-success" && "bg-success/5 border-success/30"
                )}>
                  <div className="flex items-center gap-2 mb-2">
                    <Shield className="h-4 w-4" />
                    <span className="text-sm font-medium text-foreground">Risk Level</span>
                  </div>
                  <p className={cn("text-lg font-bold", getRiskLevel(selectedToken).color)}>
                    {getRiskLevel(selectedToken).level}
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-lg bg-muted/10 border border-border/30">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-foreground">Liquidity Change (24h)</span>
                  <span className={cn(
                    "font-bold",
                    selectedToken.liquidityChange >= 0 ? "text-success" : "text-danger"
                  )}>
                    {selectedToken.liquidityChange >= 0 ? "+" : ""}{selectedToken.liquidityChange.toFixed(2)}%
                  </span>
                </div>
                <p className="text-xs text-muted-foreground">
                  {selectedToken.liquidityChange >= 0 ? "Liquidity is increasing - healthy sign" : "Liquidity is decreasing - monitor closely"}
                </p>
              </div>
              ...
              <div className="p-4 rounded-lg bg-primary/5 border border-primary/20">
                <h4 className="text-sm font-medium text-foreground mb-3 flex items-center gap-2">
                  <Info className="h-4 w-4 text-primary" /> Analysis
                </h4>
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex items-start gap-2">
                    <span className="text-primary mt-1">•</span>
                    {selectedToken.heatLevel === "hot" 
                      ? "Token is experiencing high activity - potential breakout candidate"
                      : selectedToken.heatLevel === "warm"
                      ? "Moderate activity detected - worth monitoring for entry"
                      : "Low activity - may be consolidating or building base"}
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-primary mt-1">•</span>
                    {selectedToken.volumeSpike > 70 
                      ? "Volume spike detected - institutional interest possible"
                      : "Normal volume levels"}
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-primary mt-1">•</span>
                    {selectedToken.socialScore > 60 
                      ? "High social buzz - trending in crypto communities"
                      : "Limited social discussion currently"}
                  </li>
                </ul>
              </div>
              ...
              {/* External Links */}
              <div className="flex flex-wrap gap-2">
                <a href={`https://dexscreener.com/${chain.dexScreenerId || chain.id}/${selectedToken.symbol.toLowerCase()}`} 
                ...
                <a href={`https://www.geckoterminal.com/${chain.dexScreenerId || chain.id}/pools/${selectedToken.symbol.toLowerCase()}`}
                   target="_blank" rel="noopener noreferrer"
                  ...
                  <BarChart3 className="h-4 w-4" /> GeckoTerminal
                </a>
                <a href={`https://tokensniffer.com/token/${chain.dexScreenerId || chain.id}`}
                   target="_blank" rel="noopener noreferrer"
                   className="flex items-center gap-1 px-3 py-2 rounded-lg bg-muted/20 text-foreground hover:bg-muted/30 transition-all text-sm">
                  <Shield className="h-4 w-4" /> TokenSniffer
                </a>
              </div>
            </div>
          )}
