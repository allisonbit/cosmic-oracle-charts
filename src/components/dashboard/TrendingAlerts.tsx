import { Flame, TrendingUp, TrendingDown, AlertTriangle, Rocket } from "lucide-react";
...
export function TrendingAlerts() {
  ...
  const alerts = useMemo(() => {
    ...
    return alertList.sort((a, b) => Math.abs(b.change) - Math.abs(a.change)).slice(0, 5);
  }, [data]);
        ...
        <div className="space-y-3">
          {alerts.map((alert, i) => {
            ...
            return (
              <div 
                key={`${alert.symbol}-${i}`}
                className={cn("p-3 rounded-lg border flex items-center gap-3 animate-fade-in", style.bg)}
