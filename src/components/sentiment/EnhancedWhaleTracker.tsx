            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-3">
                  <div className={cn(
                    "w-12 h-12 rounded-full flex items-center justify-center",
                    selectedAlert.type === "accumulation" ? "bg-success/20" : "bg-danger/20"
                  )}>
                    {selectedAlert.type === "accumulation" ? 
                      <TrendingUp className="w-6 h-6 text-success" /> : 
                      <TrendingDown className="w-6 h-6 text-danger" />
                    }
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn(
                        ...
                        selectedAlert.type === "accumulation" ? "bg-success/20 text-success" : "bg-danger/20 text-danger"
                      )}>
                        {selectedAlert.type.toUpperCase()}
                      </span>
                    </div>
                    <p className="text-sm text-muted-foreground font-normal">Whale Activity Details</p>
                  </div>
                </DialogTitle>
              </DialogHeader>
              ...
              <Tabs defaultValue="overview" className="mt-4">
                ...
                <TabsContent value="overview" className="space-y-4 mt-4">
                  ...
                  <div className={cn(
                    ...
                    selectedAlert.type === "accumulation" ? "bg-success/10 border-success/30" : "bg-danger/10 border-danger/30"
                  )}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-muted-foreground">Volume</span>
                      <span className="text-2xl font-bold">${selectedAlert.amount}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Clock className="w-4 h-4" />
                      <span>Detected {selectedAlert.time}</span>
                    </div>
                  </div>
                    ...
                    {selectedAlert.price && (
                      <div className="p-3 rounded-lg bg-muted/30">
                        ...
                        <p className="text-lg font-bold">
                          ${selectedAlert.price.toLocaleString()}
                        </p>
                      </div>
                    )}
                    {selectedAlert.change24h !== undefined && (
                      <div className="p-3 rounded-lg bg-muted/30">
                        <div className="flex items-center gap-2 text-muted-foreground mb-1">
                        ...
                        <p className={cn(
                          "text-lg font-bold",
                          selectedAlert.change24h >= 0 ? "text-success" : "text-danger"
                        )}>
                          {selectedAlert.change24h >= 0 ? "+" : ""}{selectedAlert.change24h.toFixed(2)}%
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <TabsContent value="transaction" className="space-y-4 mt-4">
                  ...
                  {selectedAlert.txHash && (
                    <div className="p-4 rounded-lg bg-muted/30">
                      ...
                      <div className="flex items-center gap-2">
                        <code className="flex-1 text-sm font-mono">{selectedAlert.txHash}</code>
                        <Button variant="ghost" size="icon" onClick={() => copyToClipboard(selectedAlert.txHash || '')}>
                  ...
                  {selectedAlert.fromWallet && (
                    <div className="p-4 rounded-lg bg-muted/30">
                      <div className="text-xs text-muted-foreground mb-2">From Wallet</div>
                      <div className="flex items-center gap-2">
                        <code className="flex-1 text-sm font-mono">{selectedAlert.fromWallet}</code>
                        <Button variant="ghost" size="icon" onClick={() => copyToClipboard(selectedAlert.fromWallet || '')}>
                  ...
                  {selectedAlert.toWallet && (
                    <div className="p-4 rounded-lg bg-muted/30">
                      <div className="text-xs text-muted-foreground mb-2">To Wallet</div>
                      <div className="flex items-center gap-2">
                        <code className="flex-1 text-sm font-mono">{selectedAlert.toWallet}</code>
                        <Button variant="ghost" size="icon" onClick={() => copyToClipboard(selectedAlert.toWallet || '')}>
                          <Copy className="w-4 h-4" />
                        </Button>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button variant="outline" onClick={() => window.open(`https://etherscan.io/tx/${selectedAlert.txHash}`, '_blank')}>
                      <Search className="w-4 h-4 mr-2" /> Etherscan
                    </Button>
                    <Button variant="outline" onClick={() => window.open(`https://platform.arkhamintelligence.com/explorer/tx/${selectedAlert.txHash}`, '_blank')}>
                      <Eye className="w-4 h-4 mr-2" /> Arkham
                    </Button>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="analysis" className="space-y-4 mt-4">
                  {/* AI Analysis */}
                  {(() => {
                    const insight = getWhaleInsight(selectedAlert);
                    return (
                      <>
                        <div className={cn(
                          "p-4 rounded-xl border",
                          insight.risk === 'high' ? "bg-danger/10 border-danger/30" : "bg-success/10 border-success/30"
                        )}>
                        ...
                        <div className="p-4 rounded-xl border border-primary/30 bg-primary/5">
                          <h4 className="font-bold text-sm mb-2 text-primary">Suggested Action</h4>
                          <p className="text-sm">{insight.action}</p>
                        </div>

                        <div className="p-4 rounded-lg bg-muted/30">
                          <h4 className="font-bold text-sm mb-3">Similar Patterns</h4>
                          <div className="space-y-2 text-sm">
                            <div className="flex items-center justify-between">
                              <span className="text-muted-foreground">Previous {selectedAlert.type}</span>
                              <span>2 hours ago</span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-muted-foreground">Result after 24h</span>
                              <span className={selectedAlert.type === 'accumulation' ? "text-success" : "text-danger"}>
                                {selectedAlert.type === 'accumulation' ? "+5.2%" : "-3.1%"}
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
              ...
              <p className="text-xs text-muted-foreground text-center mt-4">
                Whale activity is one indicator. Always DYOR and manage risk appropriately.
              </p>
            </>
          )}
        </DialogContent>
