...
import { TrendingUp, TrendingDown, Target, AlertTriangle, ChevronDown, Zap, Info } from "lucide-react";
  ...
  return (
    <div className="holo-card p-4 md:p-6">
      ...
      <div className="space-y-2">
        {divergences.map((div) => {
          const isExpanded = expandedDiv === div.symbol;
          return (
            <div key={div.symbol}>
              <button
                onClick={() => setExpandedDiv(isExpanded ? null : div.symbol)}
                className={cn(
              ...
              {isExpanded && (
                <div className="ml-4 mt-2 mb-2 p-4 rounded-lg bg-muted/20 border border-border/30 animate-in fade-in slide-in-from-top-1 duration-200 space-y-3">
                  <div className="text-center p-3 bg-muted/30 rounded-lg">
                    <div className="text-3xl font-display font-bold text-primary">{div.strength.toFixed(0)}%</div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-2 rounded bg-muted/30">
                      ...
                      <div className={cn("font-bold", div.priceChange >= 0 ? "text-success" : "text-danger")}>
                        {div.priceChange >= 0 ? '+' : ''}{div.priceChange.toFixed(2)}%
                      </div>
                    </div>
                    <div className="p-2 rounded bg-muted/30">
                      <div className="text-xs text-muted-foreground">Sentiment</div>
                      <div className={cn("font-bold", div.sentimentChange >= 0 ? "text-success" : "text-danger")}>
                        {div.sentimentChange >= 0 ? '+' : ''}{div.sentimentChange.toFixed(2)}%
                      </div>
                    </div>
                  </div>

                  <div className={cn("p-3 rounded-lg border", div.divergenceType === 'bullish' ? "bg-success/10 border-success/30" : "bg-danger/10 border-danger/30")}>
                    <p className="text-sm">{div.description}</p>
                  </div>

                  <Link to={`/price-prediction/${getCoinId(div)}/daily`}
                    className="block text-center text-sm text-primary hover:underline py-2">
                    View {div.symbol} Full Analysis →
                  </Link>
                </div>
              )}
            </div>
