...
import { ArrowDownUp, ArrowRight, Globe, Loader2, Zap, RefreshCw, CheckCircle2, AlertTriangle, Wallet } from "lucide-react";
...
const POPULAR_TOKENS: Record<number, { address: string; symbol: string; decimals: number }[]> = {
  1: [
...
const CHAIN_NAME_TO_ID: Record<string, number> = {
  ...
  arbitrum: 42161, optimism: 10, bsc: 56, binance: 56, avalanche: 43114, avax: 43114,
};
...
export function QuickTradeModal() {
  ...
  useEffect(() => {
    ...
    if (target.action === "sell" && target.contractAddress) {
      ...
      setBuyToken(target.contractAddress || tokens.find(t => t.symbol === "USDC")?.address || tokens[1]?.address || "");
    }
          ...
          <Tabs value={tab} onValueChange={setTab}>
            ...
            <TabsContent value="swap" className="space-y-3 mt-3">
              ...
              <div className="flex justify-center">
                <Button variant="ghost" size="icon" className="rounded-full border border-border h-8 w-8" onClick={() => { setSellToken(buyToken); setBuyToken(sellToken); setQuote(null); setPriceInfo(null); }}>
              ...
              {priceInfo && (
                <div className="text-xs text-muted-foreground bg-muted/30 rounded-lg p-2 space-y-1">
                  <div className="flex justify-between"><span>Price</span><span>1 {sellTokenInfo?.symbol} ≈ {Number(priceInfo.price || 0).toFixed(6)} {buyTokenInfo?.symbol}</span></div>
                </div>
              )}
              ...
              {!quote ? (
                <div className="flex gap-2">
                  <Button onClick={handleGetPrice} disabled={loading || !sellToken || !buyToken || !sellAmount} className="flex-1" variant="outline" size="sm">
                    {loading ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : <RefreshCw className="w-3 h-3 mr-1" />} Price
                  </Button>
                  <Button onClick={handleGetQuote} disabled={loading || !sellToken || !buyToken || !sellAmount} className="flex-1" size="sm">
